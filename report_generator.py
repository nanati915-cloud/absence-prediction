...
class ReportGenerator:
    """
    分析結果・予測結果・グラフをExcelテンプレートへ配置し、    自動で報告書を生成するクラス。

    セル位置情報はCELL_MAP、文章生成はTextBuilder、Excel操作はExcelWriterへ分離することで、帳票生成処理の保守性と変更容易性を高めている。
    """
    ...
    def run(self, results, graph_data):
        ...
        # height（高さ）を None に指定し width（横幅）のみを固定することで、
        # 元画像の縦横比を維持したままExcelへ配置する
