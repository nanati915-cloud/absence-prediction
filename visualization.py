...
def create_line_graph(df, x_col, title, filename):
    ...
    【アピール点】複数児童のデータを1つのグラフに動的プロットし、個人のノイズに囚われない全体の傾向線を一目で比較可能にしています。
    """
...
def create_future_risk_curve(result, child_name):
    ...
    【技術的工夫】ただの線グラフではなく、AIモデル独自の基準値を下回った瞬間（安定開始月）を
...
def create_future_graphs(results):
    """園児ごとに独立したデータ構造を持つAI結果をループ処理し、各児童専用の未来予測グラフを全自動生成。"""
