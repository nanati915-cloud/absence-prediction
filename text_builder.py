"""
分析結果や予測結果をもとに、Excelレポートへ出力する説明文を生成する。
データ不足時の判定や分析内容に応じた文章生成を行う。
"""
...
class TextBuilder:
    """
    分析結果や予測結果から、レポート用の説明文を生成するクラス。

    データ不足時の判定や傾向分析結果に応じて、適切な文章を自動生成する。
    """
    ...
    def future_prediction_summary(self, results):
        ...
        for child in CHILDREN:
            ...
            lower_values = result.get("future_lower", [avg])
            upper_values = result.get("future_upper", [avg])
    ...
    def business_improvement(self, results):
        ...
        for child in CHILDREN:
            ...
            if stable is not None:
                lines.append(
                    f"平均欠席日数は約{avg:.1f}日と予測された。約{stable}か月後に安定化が見込まれるが、それまでの欠席傾向を把握し、状況変化に応じた対応検討に活用することが望ましい。"
                )
            else:
                lines.append(
                    f"平均欠席日数は約{avg:.1f}日と予測された。予測期間内で安定化は見込まれないため、継続的な欠席傾向の分析や、状況変化に応じた対応検討に活用することが望ましい。"
                )
    ...
    def overall_summary(self, results):
        ...
        if stable_count == total:
            text += "全員が予測期間内に安定化すると予測され、今後も状況確認を継続することが望まれる。"
        elif stable_count == 0:
            text += "予測期間内に安定化が見込まれる児童はおらず、継続的な欠席傾向の確認と、家庭と保育園・学校との連携が重要と考えられる。"
        else:
            text += (
    ...
    def final_conclusion(self, results):
        ...
        if stable:
            ...
            return (
                ...
                "今後も欠席傾向の確認と、家庭と保育園・学校との情報共有を継続し、児童ごとの状況に応じた支援を行うことが望まれる。"
            )

        return (
            "AI予測では予測期間内に安定化が見込まれる児童は確認されなかった。"
            "継続的な欠席傾向の確認と、家庭と保育園・学校との情報共有を行いながら、個々の状況に応じた支援を継続することが望まれる。"
        )